...
<div class="form-group col-sm-12">
   ...
   <select class="form-control"  name="type" id="items_select">
    ...
    <option  value="textarea"   @if(!empty($customPostTypeItems))@if($customPostTypeItems->type=="textarea") selected @endif @endif>textarea(适用于多行文本,不可添加图片)</option>
    ...
    <option  value="select"   @if(!empty($customPostTypeItems))@if($customPostTypeItems->type=="select") selected @endif @endif>select(适用于多项预设的值,只能选一个)</option>
    <option  value="checkbox"   @if(!empty($customPostTypeItems))@if($customPostTypeItems->type=="checkbox") selected @endif @endif>checkbox(适用于多项预设的值,可选多个)</option>
    <option  value="longText"   @if(!empty($customPostTypeItems))@if($customPostTypeItems->type=="longText") selected @endif @endif>longText(适用于多行文本,可添加图片)</option>
    <option  value="file"   @if(!empty($customPostTypeItems))@if($customPostTypeItems->type=="file") selected @endif @endif>file(适用于上传文件,可添加图片等其他文件)</option>
  </select>   
</div>
