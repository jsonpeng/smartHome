...
class CategoryRepository extends BaseRepository
{
    ...
    public function getCachePostOfCat($idORslug ,$skip = 0 , $number = 6){
        return Cache::remember('zcjy_posts_of_category_id'.$idORslug.'_'.$skip.$number, Config::get('web.shrottimecache'), function() use ($idORslug, $skip, $number) {
            try {
     ...
     */
    public function getCachePostOfCatIncludeChildrenPaginate($category){
        // return Cache::remember('get_cache_post_of_cat_include_children_pagenate_'.$category->id, Config::get('web.shrottimecache'), function() use ($category) {
            try {
                ...
                $post= DB::table('posts')
                    ...
                    ->where('posts.publish_status','已通过')
                    ->orderBy('posts.is_top','desc')
     ...
     * @return [type]                  [description]
     */
    public function getCachePostOfCatIncludeChildren($category,$take=false,$skip=0){
        return Cache::remember('get_cache_post_of_cat_include_children'.$category->id.'_'.$take.'_'.$skip, Config::get('web.shrottimecache'), function() use ($category,$take,$skip) {
            try {
                $childCats = $this->getChildCatsOfCatWithParent($category);
                $cat_id_array = array();
                foreach ($childCats as $cat) {
